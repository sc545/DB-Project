...
class CtblCustomerUpdatePointAccessor
{
public:
	LONG m_cus_id;
	TCHAR m_cus_name[21];
...
// 다른 형식으로 암호를 저장하거나 다른 사용자 인증을 사용하십시오.
	...
	DEFINE_COMMAND_EX(CtblCustomerUpdatePointAccessor, L"UPDATE dbo.tblCustomer SET cus_point = cus_point + ? WHERE cus_id = ?")


	// 일부 공급자와 관련된 몇몇 문제점을 해결하기 위해 아래 코드에서는 
	// 공급자가 보고하는 것과 다른 순서로 열을 바인딩할 수 있습니다.
	BEGIN_PARAM_MAP(CtblCustomerUpdatePointAccessor)
		COLUMN_ENTRY_LENGTH_STATUS(1, m_cus_point, m_dwcus_pointLength, m_dwcus_pointStatus)
		COLUMN_ENTRY_LENGTH_STATUS(2, m_cus_id, m_dwcus_idLength, m_dwcus_idStatus)
	END_PARAM_MAP()
};
...
class CtblCustomerSelectWhereIdAccessor
{
public:
	LONG m_cus_id;
	TCHAR m_cus_name[21];
...
// 다른 형식으로 암호를 저장하거나 다른 사용자 인증을 사용하십시오.
	...
	DEFINE_COMMAND_EX(CtblCustomerSelectWhereIdAccessor, L" SELECT \
		cus_id, \
		cus_name, \
		cus_phone, \
		cus_point \
		FROM dbo.tblCustomer WHERE cus_id = ? ")


	// 일부 공급자와 관련된 몇몇 문제점을 해결하기 위해 아래 코드에서는 
	// 공급자가 보고하는 것과 다른 순서로 열을 바인딩할 수 있습니다.

	BEGIN_PARAM_MAP(CtblCustomerSelectWhereIdAccessor)
		COLUMN_ENTRY_LENGTH_STATUS(1, m_cus_id, m_dwcus_idLength, m_dwcus_idStatus)
	END_PARAM_MAP()

	BEGIN_COLUMN_MAP(CtblCustomerSelectWhereIdAccessor)
		COLUMN_ENTRY_LENGTH_STATUS(1, m_cus_id, m_dwcus_idLength, m_dwcus_idStatus)
		COLUMN_ENTRY_LENGTH_STATUS(2, m_cus_name, m_dwcus_nameLength, m_dwcus_nameStatus)
		COLUMN_ENTRY_LENGTH_STATUS(3, m_cus_phone, m_dwcus_phoneLength, m_dwcus_phoneStatus)
		COLUMN_ENTRY_LENGTH_STATUS(4, m_cus_point, m_dwcus_pointLength, m_dwcus_pointStatus)
